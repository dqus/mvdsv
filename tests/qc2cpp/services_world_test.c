...
edict_t *QC_SlotToEdict(qc_entity_id_t slot) { return slot == 0U ? &test_entity : slot == 1U ? &test_spawned : NULL; }
qc_entity_id_t QC_EdictToSlot(const edict_t *entity) { return entity == &test_entity ? 0U : entity == &test_spawned ? 1U : QC_INVALID_ENTITY_ID; }
...
int QC_SetEntityString(edict_t *entity, const char *field, const char *value) { assert(entity == &test_entity); assert(!strcmp(field, "model")); strlcpy(entity_model, value, sizeof(entity_model)); return 1; }
qc_plugin_status_t QC_CopyEntityString(const edict_t *entity, const char *field, char *out, uint32_t capacity, uint32_t *required) { assert(entity == &test_entity); assert(!strcmp(field, "model")); assert(capacity > strlen(entity_model)); strcpy(out, entity_model); if (required != NULL) *required = (uint32_t)strlen(entity_model) + 1U; return QC_PLUGIN_OK; }
...
static void test_fatal(void *context, const qc_program_diagnostic_v1_t *diagnostic)
...
int main(void)
{
	...
	assert(host.map_metadata(host.context, 0U, (const uint8_t *)"unknown", 7U,
		(const uint8_t *)"value", 5U) == QC_MAP_METADATA_NOT_HANDLED);
	assert(host.map_admit(host.context, 0U, 0.0f) == QC_MAP_ACCEPT);
	...
	assert(host.map_admit(host.context, 0U, (float)SPAWNFLAG_NOT_MEDIUM)
		== QC_MAP_REJECT);
	QC_BindUnavailableServices(&host);
	assert(qc_validate_host_api_v1(&host) == QC_PLUGIN_OK);
	return 0;
}
