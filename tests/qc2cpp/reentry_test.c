...
static qc_entity_id_t observed_first;
static qc_entity_id_t observed_second;
static float observed_time;
static float observed_frametime;
static qc_shared_global_state_v1_t globals;
...
qc_plugin_status_t QC_CopyEntityString(const edict_t *entity, const char *field,
...
qc_entity_id_t QC_EdictToSlot(const edict_t *entity)
{
	if (entity >= entities && entity < entities + 8) {
		return (qc_entity_id_t)(entity - entities);
	}
...
qc_shared_global_state_v1_t *QC_Globals(void) { return &globals; }
qbool QC_Active(void) { return true; }

void QC_EdictTouch(qc_entity_id_t first, qc_entity_id_t second, float time,
	float frametime)
{
...
{
	QC_EdictTouch(self, QC_INVALID_ENTITY_ID, time, frametime);
	++think_call_count;
}

void QC_EdictBlocked(qc_entity_id_t first, qc_entity_id_t second, float time,
	float frametime)
{
...
int main(void)
{
	...
	sv.max_edicts = 8;
	QC_DispatchEdictTouch(&entities[3], &entities[4], 17.0f, 0.125f);
	assert(observed_first == 3U && observed_second == 4U);
	assert(observed_time == 17.0f && observed_frametime == 0.125f);

	QC_DispatchEdictThink(&entities[5], 19.0f, 0.25f);
	assert(observed_first == 5U && observed_second == QC_INVALID_ENTITY_ID);
	assert(observed_time == 19.0f && observed_frametime == 0.25f);

	QC_DispatchEdictBlocked(&entities[6], &entities[7], 23.0f, 0.5f);
	assert(observed_first == 6U && observed_second == 7U);
	assert(observed_time == 23.0f && observed_frametime == 0.5f);

	qc_host_api_v1_t host = {0};
	QC_BindMovementServices(&host);
