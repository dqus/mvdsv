...
{
	qc_save_image_t *image = (qc_save_image_t *)(uintptr_t)1;
	assert(QC_SaveParse(bytes, size, &image) != QC_PLUGIN_OK);
	assert(image == NULL);
}
...
static void test_parses_and_reencodes_a_transport_independent_image(void)
{
	...
	qc_save_image_t *round_trip = NULL;
	...
	assert(QC_SaveEncode(image, &encoded, &encoded_size) == QC_PLUGIN_OK);
	assert(encoded_size == input.size);
	assert(memcmp(encoded, input.bytes, input.size) == 0);
	assert(QC_SaveParse(encoded, encoded_size, &round_trip) == QC_PLUGIN_OK);
	assert(round_trip->metadata.entity_capacity == fixture_capacity);
	QC_SaveImageFree(round_trip);
	free(encoded);
