...
int main(int argc, char **argv)
{
	...
	assert(QCX_TransportOpen((qcx_transport_kind_t)99, ".", "game", NULL, &transport, &diagnostic)
	       == QCX_PLUGIN_BAD_ARGUMENT);
	assert(transport == NULL);
	assert(QCX_TransportOpen(QCX_TRANSPORT_NONE, ".", "game", NULL, &transport, &diagnostic)
	       == QCX_PLUGIN_BAD_ARGUMENT);
	assert(transport == NULL);
	assert(QCX_TransportOpen(QCX_TRANSPORT_NATIVE, ".", "game", NULL, &transport, &diagnostic)
	       == QCX_PLUGIN_BAD_ARGUMENT);
	assert(transport == NULL && diagnostic.message_size != 0U);
	assert(QCX_TransportOpen(QCX_TRANSPORT_NATIVE, "/definitely/missing", "game", NULL,
	...
	assert(QCX_TransportOpen(QCX_TRANSPORT_NATIVE, argv[1], "bad", NULL, &transport, &diagnostic)
	       == QCX_PLUGIN_BAD_ABI);
	assert(transport == NULL && diagnostic.message_size != 0U);
	assert(QCX_TransportOpen(QCX_TRANSPORT_NATIVE, argv[1], "no_query", NULL, &transport, &diagnostic)
...
#endif
	assert(QCX_TransportOpen(QCX_TRANSPORT_WASM, argv[1], "game", NULL, &transport, &diagnostic)
	       == wasm_missing_status);
	assert(transport == NULL && diagnostic.message_size != 0U);
	return 0;
}
